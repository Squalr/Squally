...
AsmLabel CodeBuilder::newNamedLabel(const char* name, size_t nameLength, uint32_t type, uint32_t parentId) {
  uint32_t id = kInvalidValue;
...
Error CodeBuilder::bind(const AsmLabel& label) {
  if (_lastError) return _lastError;
...
Error CodeBuilder::embedLabel(const AsmLabel& label) {
  if (_lastError) return _lastError;
...
Error CodeBuilder::embedConstPool(const AsmLabel& label, const ConstPool& pool) {
  if (_lastError) return _lastError;
