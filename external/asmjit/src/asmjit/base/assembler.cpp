...
AsmLabel Assembler::newNamedLabel(const char* name, size_t nameLength, uint32_t type, uint32_t parentId) {
  uint32_t id = 0;
  if (!_lastError) {
    ASMJIT_ASSERT(_code != nullptr);
...
Error Assembler::bind(const AsmLabel& label) {
  if (_lastError) return _lastError;
  ASMJIT_ASSERT(_code != nullptr);
...
Error Assembler::embedLabel(const AsmLabel& label) {
  if (_lastError) return _lastError;
  ASMJIT_ASSERT(_code != nullptr);
...
Error Assembler::embedConstPool(const AsmLabel& label, const ConstPool& pool) {
  if (_lastError) return _lastError;
