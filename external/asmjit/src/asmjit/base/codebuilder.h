...
public:
  ...
  ASMJIT_INLINE Error getCBLabel(CBLabel** pOut, const AsmLabel& label) noexcept { return getCBLabel(pOut, label.getId()); }
  ...
  ASMJIT_API virtual AsmLabel newLabel() override;
  ASMJIT_API virtual AsmLabel newNamedLabel(const char* name, size_t nameLength = Globals::kInvalidIndex, uint32_t type = AsmLabel::kTypeGlobal, uint32_t parentId = kInvalidValue) override;
  ASMJIT_API virtual Error bind(const AsmLabel& label) override;
  ...
  ASMJIT_API virtual Error embedLabel(const AsmLabel& label) override;
  ASMJIT_API virtual Error embedConstPool(const AsmLabel& label, const ConstPool& pool) override;
...
public:
  ...
  ASMJIT_INLINE AsmLabel getLabel() const noexcept { return AsmLabel(_id); }
