////////////////////////////////////////////////////////////////////////////////////////////
// THIS C++ FILE IS GENERATED DO NOT EDIT. RUN GenerateResources.py TO GENERATE THIS FILE //
////////////////////////////////////////////////////////////////////////////////////////////
#pragma once
#include <string>

namespace IsometricMapResources
{
	extern const std::string BalmerPeaks_Map0;
	extern const std::string DaemonsHallow_Map0;
	extern const std::string EndianForest_Map0;
	extern const std::string EndianForest_Map1;
	extern const std::string EndianForest_Map10;
	extern const std::string EndianForest_Map11;
	extern const std::string EndianForest_Map12;
	extern const std::string EndianForest_Map13;
	extern const std::string EndianForest_Map14;
	extern const std::string EndianForest_Map15;
	extern const std::string EndianForest_Map16;
	extern const std::string EndianForest_Map17;
	extern const std::string EndianForest_Map18;
	extern const std::string EndianForest_Map19;
	extern const std::string EndianForest_Map2;
	extern const std::string EndianForest_Map20;
	extern const std::string EndianForest_Map21;
	extern const std::string EndianForest_Map22;
	extern const std::string EndianForest_Map23;
	extern const std::string EndianForest_Map3;
	extern const std::string EndianForest_Map4;
	extern const std::string EndianForest_Map5;
	extern const std::string EndianForest_Map6;
	extern const std::string EndianForest_Map7;
	extern const std::string EndianForest_Map8;
	extern const std::string EndianForest_Map9;
	extern const std::string EndianForest_MapEmpty;
	extern const std::string LevelSelectMap;
	extern const std::string Overworld;
	extern const std::string Sanctum;
	extern const std::string SquallyHome;
	extern const std::string TestLevel;
	extern const std::string TheDarkWeb;
	extern const std::string TheInternet;
	extern const std::string TileMap;
};
