...
#include "Objects/Platformer/Combat/Projectiles/ShadowBomb/ShadowBombGenericPreview.h"
#include "Objects/Platformer/Combat/Projectiles/ShadowBomb/ShadowBombSpeedPreview.h"
...
ShadowBomb* ShadowBomb::create(PlatformerEntity* owner, PlatformerEntity* target)
{
	ShadowBomb* instance = new ShadowBomb(owner, target);
...
ShadowBomb::ShadowBomb(PlatformerEntity* owner, PlatformerEntity* target)
...
void ShadowBomb::update(float dt)
{
	...
	this->setShadowBombSpeed();

	this->shadowBoltAnim->setFlippedX(this->getLaunchVelocity().x < 0.0f);
}

void ShadowBomb::runSpawnFX()
{
	this->launchSound->play();

	this->shadowBoltAnim->playAnimationRepeat(FXResources::ShadowBomb_ShadowBomb_0000, 0.05f);
}

void ShadowBomb::runImpactFX()
{
...
void ShadowBomb::registerHackables()
{
	...
	{
		{
			...
			HackableCode::HackableCodeInfo(
				...
				LazyNode<HackablePreview>::create([=](){ return ShadowBombSpeedPreview::create(); }),
				{
	...
	auto shadowBombFunc = &ShadowBomb::setShadowBombSpeed;
	std::vector<HackableCode*> hackables = HackableCode::create((void*&)shadowBombFunc, codeInfoMap);
...
NO_OPTIMIZE void ShadowBomb::setShadowBombSpeed()
{
