...
VaporWeb::VaporWeb()
{
	this->cells = std::map<int, GridItem*>();
	...
	for (int column = 0; column < VaporWeb::cellColumns; column++)
	{
		for (int row = 0; row < VaporWeb::cellRows; row++)
		{
			...
			GridItem* cell = GridItem::create(Sprite::create(CutsceneResources::VaporWeb_Cell), Vec2(gridRow, gridColumn), true);
			cell->setOpacity(0);

			this->cells[this->getCellIndex(row, column)] = cell;

			this->grid->addGridItem(cell);
		}
	}
...
FiniteTimeAction* VaporWeb::createCutsceneGridSetup()
{
	...
	CallFunc* fadeInCells = CallFunc::create([=]()
	{
		...
		for (int column = 0; column < VaporWeb::cellColumns; column++)
		{
			for (int row = 0; row < VaporWeb::cellRows; row++)
			{
				GridItem* cell = this->cells.at(this->getCellIndex(row, column));
...
FiniteTimeAction* VaporWeb::createCutsceneForest()
{
	...
	{
		this->grid->addGridItem(GridItem::create(Sprite::create(CutsceneResources::VaporWeb_Forest_Tree1), Vec2(-2.0f, Grid::lineColumns / 2.0f - 3.5f), false));
		this->grid->addGridItem(GridItem::create(Sprite::create(CutsceneResources::VaporWeb_Forest_Tree2), Vec2(-3.0f, Grid::lineColumns / 2.0f + 3.0f), false));
		this->grid->addGridItem(GridItem::create(Sprite::create(CutsceneResources::VaporWeb_Forest_Shrine), Vec2(-10.0f, Grid::lineColumns / 2.0f + 0.0f), false));
		this->grid->addGridItem(GridItem::create(Sprite::create(CutsceneResources::VaporWeb_Forest_Ruins1), Vec2(-12.0f, Grid::lineColumns / 2.0f - 4.0f), false));
		this->grid->addGridItem(GridItem::create(Sprite::create(CutsceneResources::VaporWeb_Forest_Thorns1), Vec2(-15.0f, Grid::lineColumns / 2.0f - 2.0f), false));
	});
...
FiniteTimeAction* VaporWeb::createCutsceneCaverns()
{
	...
	CallFunc* addObjects = CallFunc::create([=]()
	{
		this->grid->addGridItem(GridItem::create(Sprite::create(CutsceneResources::VaporWeb_Caverns_DryTree1), Vec2(-2.0f, Grid::lineColumns / 2.0f - 3.5f), false));
		this->grid->addGridItem(GridItem::create(Sprite::create(CutsceneResources::VaporWeb_Caverns_DryTree2), Vec2(-3.0f, Grid::lineColumns / 2.0f + 3.0f), false));
		this->grid->addGridItem(GridItem::create(Sprite::create(CutsceneResources::VaporWeb_Caverns_Skulls), Vec2(-10.0f, Grid::lineColumns / 2.0f + 0.0f), false));
	});
...
FiniteTimeAction* VaporWeb::createCutsceneObelisk()
{
	...
	CallFunc* addObjects = CallFunc::create([=]()
	{
		this->grid->addGridItem(GridItem::create(Sprite::create(CutsceneResources::VaporWeb_Obelisk_Tree1), Vec2(-2.0f, Grid::lineColumns / 2.0f - 3.5f), false));
		this->grid->addGridItem(GridItem::create(Sprite::create(CutsceneResources::VaporWeb_Obelisk_Door), Vec2(-3.0f, Grid::lineColumns / 2.0f + 3.0f), false));
		this->grid->addGridItem(GridItem::create(Sprite::create(CutsceneResources::VaporWeb_Obelisk_Obelisk), Vec2(-9.0f, Grid::lineColumns / 2.0f - 2.0f), false));
		this->grid->addGridItem(GridItem::create(Sprite::create(CutsceneResources::VaporWeb_Obelisk_Tree2), Vec2(-12.0f, Grid::lineColumns / 2.0f + 3.0f), false));
		this->grid->addGridItem(GridItem::create(Sprite::create(CutsceneResources::VaporWeb_Obelisk_Wall), Vec2(-13.0f, Grid::lineColumns / 2.0f - 4.0f), false));
	});
