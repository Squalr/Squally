...
void StateLoadInitialState::loadStateOverrides(GameState* gameState, StateOverride* stateOverride)
{
	...
	{
		gameState->playerHand->insertCard(Card::create(Card::CardStyle::Earth, *it, true), 0.0f);
	}

	for (auto it = stateOverride->playerDeck.begin(); it != stateOverride->playerDeck.end(); it++)
	{
		gameState->playerDeck->insertCardBottom(Card::create(Card::CardStyle::Earth, *it, true), false, 0.0f);
	}

	for (auto it = stateOverride->playerBinaryCards.begin(); it != stateOverride->playerBinaryCards.end(); it++)
	{
		gameState->playerBinaryCards->insertCard(Card::create(Card::CardStyle::Earth, *it, true), 0.0f);
	}

	for (auto it = stateOverride->playerDecimalCards.begin(); it != stateOverride->playerDecimalCards.end(); it++)
	{
		gameState->playerDecimalCards->insertCard(Card::create(Card::CardStyle::Earth, *it, true), 0.0f);
	}

	for (auto it = stateOverride->playerHexCards.begin(); it != stateOverride->playerHexCards.end(); it++)
	{
		gameState->playerHexCards->insertCard(Card::create(Card::CardStyle::Earth, *it, true), 0.0f);
	}

	// Load enemy cards
	for (auto it = stateOverride->enemyHand.begin(); it != stateOverride->enemyHand.end(); it++)
	{
		gameState->enemyHand->insertCard(Card::create(Card::CardStyle::Shadow, *it, false), 0.0f);
	}

	for (auto it = stateOverride->enemyDeck.begin(); it != stateOverride->enemyDeck.end(); it++)
	{
		gameState->enemyDeck->insertCardBottom(Card::create(Card::CardStyle::Shadow, *it, false), false, 0.0f);
	}

	for (auto it = stateOverride->enemyBinaryCards.begin(); it != stateOverride->enemyBinaryCards.end(); it++)
	{
		gameState->enemyBinaryCards->insertCard(Card::create(Card::CardStyle::Shadow, *it, false), 0.0f);
	}

	for (auto it = stateOverride->enemyDecimalCards.begin(); it != stateOverride->enemyDecimalCards.end(); it++)
	{
		gameState->enemyDecimalCards->insertCard(Card::create(Card::CardStyle::Shadow, *it, false), 0.0f);
	}

	for (auto it = stateOverride->enemyHexCards.begin(); it != stateOverride->enemyHexCards.end(); it++)
	{
		gameState->enemyHexCards->insertCard(Card::create(Card::CardStyle::Shadow, *it, false), 0.0f);
	}
