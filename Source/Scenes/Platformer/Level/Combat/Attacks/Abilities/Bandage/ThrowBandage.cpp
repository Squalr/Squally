...
const float ThrowBandage::HealPercentage = 0.75f;

ThrowBandage* ThrowBandage::create(Priority priority)
{
	ThrowBandage* instance = new ThrowBandage(priority);
...
ThrowBandage::ThrowBandage(Priority priority)
	: super(AttackType::Healing, ObjectResources::Bandages_Bandage, priority, AbilityType::Physical, 10, 15, 0, 0.2f, 1.5f)
{
...
ThrowBandage::~ThrowBandage()
{
}

PlatformerAttack* ThrowBandage::cloneInternal()
{
	return ThrowBandage::create(this->priority);
}

LocalizedString* ThrowBandage::getString()
{
	return Strings::Items_Consumables_Health_Bandage::create();
}

LocalizedString* ThrowBandage::getDescription()
{
	return Strings::Items_Consumables_Health_BandageDescription::create()
		->setStringReplacementVariables(ConstantString::create(std::to_string(int(ThrowBandage::HealPercentage * 100.0f))));
}

std::string ThrowBandage::getAttackAnimation()
{
	return "ThrowItem";
}

void ThrowBandage::onAttackTelegraphBegin()
{
...
void ThrowBandage::performAttack(PlatformerEntity* owner, std::vector<PlatformerEntity*> targets)
{
	...
	for (PlatformerEntity* next : targets)
	{
		...
		potion->whenCollidesWith({ (int)CombatCollisionType::EntityEnemy, (int)CombatCollisionType::EntityFriendly }, [=](CollisionData collisionData)
		{
			...
			if (entity != nullptr)
			{
				int healing = int(std::round(float(entity->getRuntimeStateOrDefaultInt(StateKeys::MaxHealth, 0))) * ThrowBandage::HealPercentage);
