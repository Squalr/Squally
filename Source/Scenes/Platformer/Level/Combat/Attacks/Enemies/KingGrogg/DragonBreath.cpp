...
void DragonBreath::performAttack(PlatformerEntity* owner, std::vector<PlatformerEntity*> targets)
{
	...
	for (auto next : targets)
	{
		...
		Vec2 entityOffset = flipMultiplier * owner->getEntitySize() / 2.0f;
		fireball->setPosition3D(GameUtils::getWorldCoords3D(owner) + Vec3(flipMultiplier * 96.0f - entityOffset.x, 96.0f - entityOffset.y, 0.0f));
