////////////////////////////////////////////////////////////////////////////////////////////
// THIS C++ FILE IS GENERATED DO NOT EDIT. RUN GenerateDataFiles.py TO GENERATE THIS FILE //
////////////////////////////////////////////////////////////////////////////////////////////

#include "ScoreTotals.h"

namespace Strings
{
	Hexus_Tutorials_A_ScoreTotals* Hexus_Tutorials_A_ScoreTotals::create()
	{
		Hexus_Tutorials_A_ScoreTotals* instance = new Hexus_Tutorials_A_ScoreTotals();

		instance->autorelease();

		return instance;
	}
	
	LocalizedString* Hexus_Tutorials_A_ScoreTotals::clone()
	{
		LocalizedString* clonedString = Hexus_Tutorials_A_ScoreTotals::create();
		
		this->copyAttributesTo(clonedString);
		
		return clonedString;
	}

	Hexus_Tutorials_A_ScoreTotals::Hexus_Tutorials_A_ScoreTotals() : LocalizedString()
	{
	}

	Hexus_Tutorials_A_ScoreTotals::~Hexus_Tutorials_A_ScoreTotals()
	{
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringIdentifier()
	{
		return "Hexus_Tutorials_A_ScoreTotals";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringAr()
	{
		return "الهدف بسيط. كل من حصل على أعلى نتيجة في نهاية الجولة يفوز بالجولة.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringBg()
	{
		return "Целта е проста. Който има по-висок резултат в края на кръг, печели кръга.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringCs()
	{
		return "Cíl je jednoduchý. Kdo má nejvyšší skóre na konci kola, vyhraje kolo.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringDa()
	{
		return "Målet er simpelt. Den, der har den højere score i slutningen af ​​en runde vinder runden.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringDe()
	{
		return "Das Ziel ist einfach. Wer am Ende einer Runde die höhere Punktzahl hat, gewinnt die Runde.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringEl()
	{
		return "Ο στόχος είναι απλός. Όποιος έχει το υψηλότερο σκορ στο τέλος ενός γύρου κερδίζει το γύρο.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringEn()
	{
		return "The objective is simple. Whoever has the higher score at the end of a round wins the round.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringEs()
	{
		return "El objetivo es simple. Quien tenga la puntuación más alta al final de una ronda gana la ronda.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringEs419()
	{
		return "El objetivo es simple. Quien tenga la puntuación más alta al final de una ronda gana la ronda.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringFi()
	{
		return "Tavoitteena on yksinkertainen. Kuka tahansa, jolla on suurempi pisteet kierroksen lopussa, voittaa kierroksen.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringFr()
	{
		return "L'objectif est simple. Celui qui a le score le plus élevé à la fin d'une manche remporte la manche.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringHu()
	{
		return "A cél egyszerű. Aki a forduló végén magasabb pontszámot kap, megnyeri a fordulót.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringIt()
	{
		return "L'obiettivo è semplice. Chi ha il punteggio più alto alla fine di un round vince il round.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringJa()
	{
		return "目的は簡単です。ラウンドの終わりに高い得点を得た人がそのラウンドに勝ちます。";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringKo()
	{
		return "목적은 간단합니다. 라운드가 끝나면 높은 점수를받은 사람은 라운드에서 승리합니다.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringNl()
	{
		return "Het doel is eenvoudig. Wie aan het einde van een ronde de hoogste score heeft, wint de ronde.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringNo()
	{
		return "Målet er enkelt. Den som har høyere poengsum på slutten av en runde vinner runden.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringPl()
	{
		return "Cel jest prosty. Ten, kto ma wyższą liczbę punktów na koniec rundy, wygrywa rundę.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringPt()
	{
		return "O objetivo é simples. Quem tiver a maior pontuação no final de uma rodada ganha a rodada.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringPtBr()
	{
		return "O objetivo é simples. Quem tiver a maior pontuação no final de uma rodada ganha a rodada.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringRo()
	{
		return "Obiectivul este simplu. Oricine are scorul cel mai mare la sfârșitul unei runde câștigă runda.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringRu()
	{
		return "Цель проста. Тот, кто имеет более высокий балл в конце раунда, выигрывает раунд.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringSv()
	{
		return "Målet är enkelt. Den som har högre poäng i slutet av en runda vinner rundan.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringTh()
	{
		return "วัตถุประสงค์ง่าย ๆ ใครก็ตามที่มีคะแนนสูงกว่าในตอนท้ายของรอบชนะรอบ";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringTr()
	{
		return "Amaç basit. Bir turun sonunda en yüksek puanı alan kişi, turu kazanır.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringUk()
	{
		return "Мета проста. Той, хто має вищий бал в кінці раунду, виграє раунд.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringVi()
	{
		return "Mục tiêu rất đơn giản. Ai có số điểm cao hơn ở cuối vòng thì sẽ thắng vòng đó.";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringZhCn()
	{
		return "目标很简单。在一轮结束时获得较高分数的人赢得该轮。";
	}

	std::string Hexus_Tutorials_A_ScoreTotals::getStringZhTw()
	{
		return "目標很簡單。在一輪結束時獲得較高分數的人贏得該輪。";
	}
}
