...
void RadialMenu::onHackableEdit(EventCustom* eventArgs)
{
	...
	line->drawLine(args->sourceLocation - Director::getInstance()->getVisibleSize() / 2.0f, Vec2::ZERO, RadialMenu::dataColor);
	...
	for (auto it = this->activeHackableObject->dataList->begin(); it != this->activeHackableObject->dataList->end(); it++)
	{
		...
		for (auto it = hackableData->codeList->begin(); it != hackableData->codeList->end(); it++)
		{
			...
			nextCodeIconBack->drawCircle(Vec2::ZERO, RadialMenu::iconRadius, 0.0f, 64, false, RadialMenu::codeColor);
			...
			nextCodeIconLine->drawLine(Vec2::ZERO, nextCodeLineEndPosition, RadialMenu::codeColor);
		...
		nextDataIconBack->drawCircle(Vec2::ZERO, RadialMenu::iconRadius, 0.0f, 64, false, RadialMenu::dataColor);
		...
		nextDataIconLine->drawLine(Vec2::ZERO, nextDataLineEndPosition, RadialMenu::dataColor);
	...
	DrawNode* returnButtonBack = DrawNode::create(RadialMenu::lineWidth);
	returnButtonBack->drawCircle(Vec2::ZERO, RadialMenu::iconRadius, 0.0f, 64, false, RadialMenu::dataColor);
	Sprite* returnIcon = Sprite::create(Resources::Menus_HackerModeMenu_Icons_Flag);
	Node* clickableNode = Node::create();
	clickableNode->setContentSize(Size(RadialMenu::iconRadius * 2.0f, RadialMenu::iconRadius * 2.0f));
	MenuSprite* returnMenuSprite = MenuSprite::create(clickableNode, Node::create(), Node::create());

	returnMenuSprite->setClickCallback(CC_CALLBACK_1(RadialMenu::onClose, this));

	this->radialNode->addChild(returnButtonBack);
	this->radialNode->addChild(returnIcon);
	this->radialNode->addChild(returnMenuSprite);
