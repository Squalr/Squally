...
public:
  ...
  ASMJIT_INLINE AsmLabel() noexcept : Operand(NoInit) { reset(); }
  //! Create a reference to another label.
  ASMJIT_INLINE AsmLabel(const AsmLabel& other) noexcept : Operand(other) {}

  explicit ASMJIT_INLINE AsmLabel(uint32_t id) noexcept : Operand(NoInit) {
    _init_packed_d0_d1(kOpLabel, id);
    _init_packed_d2_d3(0, 0);
  }

  explicit ASMJIT_INLINE AsmLabel(const _NoInit&) noexcept : Operand(NoInit) {}
  ...
  ASMJIT_INLINE AsmLabel& operator=(const AsmLabel& other) noexcept { copyFrom(other); return *this; }
};
...
public:
  ...
  ASMJIT_INLINE bool hasBaseReg() const noexcept { return (_signature & kSignatureMemBaseTypeMask) > (AsmLabel::kLabelTag << kSignatureMemBaseTypeShift); }
  //! Get if the BASE operand is a label.
  ASMJIT_INLINE bool hasBaseLabel() const noexcept { return (_signature & kSignatureMemBaseTypeMask) == (AsmLabel::kLabelTag << kSignatureMemBaseTypeShift); }
  //! Get if the INDEX operand is a register (registers start after `kLabelTag`).
  ASMJIT_INLINE bool hasIndexReg() const noexcept { return (_signature & kSignatureMemIndexTypeMask) > (AsmLabel::kLabelTag << kSignatureMemIndexTypeShift); }
