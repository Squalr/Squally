...
public:
  ...
  ASMJIT_INLINE X86Mem(const AsmLabel& base, int32_t off, uint32_t size = 0, uint32_t flags = 0) noexcept : Mem(NoInit) {
    uint32_t signature = (AsmLabel::kLabelTag << kSignatureMemBaseTypeShift ) |
  ...
  ASMJIT_INLINE X86Mem(const AsmLabel& base, const Reg& index, uint32_t shift, int32_t off, uint32_t size = 0, uint32_t flags = 0) noexcept : Mem(NoInit) {
...
static ASMJIT_INLINE X86Mem ptr(const AsmLabel& base, int32_t offset = 0, uint32_t size = 0) noexcept {
  return X86Mem(base, offset, size);
}
//! Create a `[base + (index << shift) + offset]` memory operand.
static ASMJIT_INLINE X86Mem ptr(const AsmLabel& base, const X86Gp& index, uint32_t shift, int32_t offset = 0, uint32_t size = 0) noexcept {
  return X86Mem(base, index, shift, offset, size);
}
//! Create a `[base + (index << shift) + offset]` memory operand.
static ASMJIT_INLINE X86Mem ptr(const AsmLabel& base, const X86Vec& index, uint32_t shift, int32_t offset = 0, uint32_t size = 0) noexcept {
  return X86Mem(base, index, shift, offset, size);
}
...
#define ASMJIT_X86_PTR_FN(FUNC, SIZE)                                                 \
  ...
  /*! Create a `[base + offset]` memory operand. */                                   \
  static ASMJIT_INLINE X86Mem FUNC(const AsmLabel& base, int32_t offset = 0) noexcept {  \
    return X86Mem(base, offset, SIZE);                                                \
  }                                                                                   \
  /*! Create a `[base + (index << shift) + offset]` memory operand. */                \
  static ASMJIT_INLINE X86Mem FUNC(const AsmLabel& base, const X86Gp& index, uint32_t shift, int32_t offset = 0) noexcept { \
    return X86Mem(base, index, shift, offset, SIZE);                                  \
  }                                                                                   \
