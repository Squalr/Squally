...
struct X86EmitterExplicitT {
  ...
  //! \overload
  ASMJIT_INLINE X86Mem intptr_ptr(const AsmLabel& base, int32_t offset = 0) const noexcept {
    uint32_t nativeGpSize = static_cast<const This*>(this)->getGpSize();
    return X86Mem(base, offset, nativeGpSize);
  }
  //! \overload
  ASMJIT_INLINE X86Mem intptr_ptr(const AsmLabel& base, const X86Gp& index, uint32_t shift, int32_t offset = 0) const noexcept {
    uint32_t nativeGpSize = static_cast<const This*>(this)->getGpSize();
    return X86Mem(base, index, shift, offset, nativeGpSize);
  }
  //! \overload
  ASMJIT_INLINE X86Mem intptr_ptr(const AsmLabel& base, const X86Vec& index, uint32_t shift, int32_t offset = 0) const noexcept {
    uint32_t nativeGpSize = static_cast<const This*>(this)->getGpSize();
    return X86Mem(base, index, shift, offset, nativeGpSize);
  }
  //! \overload
...
public:
  ...
  ASMJIT_INST_1x(call, Call, AsmLabel)                                           // ANY
  ...
  ASMJIT_INST_1c(j, J, X86Inst::condToJcc, AsmLabel)                             // ANY
  ASMJIT_INST_1c(j, J, X86Inst::condToJcc, Imm)                               // ANY
  ASMJIT_INST_1c(j, J, X86Inst::condToJcc, uint64_t)                          // ANY
  ASMJIT_INST_2x(jecxz, Jecxz, X86Gp, AsmLabel)                                  // ANY       [EXPLICIT] Short jump if CX/ECX/RCX is zero.
  ...
  ASMJIT_INST_1x(jmp, Jmp, AsmLabel)                                             // ANY
  ...
  ASMJIT_INST_2x(loop, Loop, ZCX, AsmLabel)                                      // ANY       [EXPLICIT] Decrement xCX; short jump if xCX != 0.
  ...
  ASMJIT_INST_2x(loope, Loope, ZCX, AsmLabel)                                    // ANY       [EXPLICIT] Decrement xCX; short jump if xCX != 0 && ZF == 1.
  ...
  ASMJIT_INST_2x(loopne, Loopne, ZCX, AsmLabel)                                  // ANY       [EXPLICIT] Decrement xCX; short jump if xCX != 0 && ZF == 0.
  ...
  ASMJIT_INST_1x(xbegin, Xbegin, AsmLabel)                                       // RTM
...
struct X86EmitterImplicitT : public X86EmitterExplicitT<This> {
  ...
  ASMJIT_INST_1x(jecxz, Jecxz, AsmLabel)                                         // ANY       [IMPLICIT] Short jump if CX/ECX/RCX is zero.
  ...
  ASMJIT_INST_1x(loop, Loop, AsmLabel)                                           // ANY       [IMPLICIT] Decrement xCX; short jump if xCX != 0.
  ...
  ASMJIT_INST_1x(loope, Loope, AsmLabel)                                         // ANY       [IMPLICIT] Decrement xCX; short jump if xCX != 0 && ZF == 1.
  ...
  ASMJIT_INST_1x(loopne, Loopne, AsmLabel)                                       // ANY       [IMPLICIT] Decrement xCX; short jump if xCX != 0 && ZF == 0.
